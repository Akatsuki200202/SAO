...
def obtain_attach_nodes_by_cluster(args,y,y_pred, model, node_idxs, x, labels, device, size):
    ...
    for id in range(x.shape[0]):
        tmp_center_label = int(y_pred[id])
        idx_tc_label = np.where(y==tmp_center_label)
        tmp_tar_label = args.target_class
        idx_tr_label = np.where(y==tmp_tar_label)
        
        #tmp_center_x = cluster_centers[tmp_center_label]
        tmp_center_x = cluster_centers[idx_tc_label]
        #tmp_tar_x = cluster_centers[tmp_tar_label]
        tmp_tar_x = cluster_centers[idx_tr_label]
    ...
    for i in label_list:
        labels_dict[i] = np.where(y_pred==i)[0]
        # filter out labeled nodes
        new_value = np.array(list(set(node_idxs) & set(labels_dict[i])))
        if(len(new_value)!=0):
            labels_dict[i] = new_value
        else:
            del labels_dict[i]
...
def cluster_distance_selection(args,data,idx_train,idx_val,idx_clean_test,train_edge_index,size,device):
    ...
    if(os.path.exists(encoder_modelpath)):
        ...
        gcn_encoder = GCN_Encoder(nfeat = data.x.shape[1],
                            ...
                            device = device,
                            use_ln=False,
                            layer_norm_first = False).to(device)
        t_total = time.time()
        # edge_weights = torch.ones([data.edge_index.shape[1]],device=device,dtype=torch.float)
        print("Length of training set: {}".format(len(idx_train)))
        gcn_encoder.fit(data.x, train_edge_index, None, data.y, idx_train, idx_val,train_iters=200, verbose=True)
    ...
    seen_node_idx = torch.concat([idx_train,idx_val])
    ...
    n_y_pred =np.unique(y_pred).shape[0]
    y_table = np.unique(data.y.cpu().numpy())
    y_table = np.sort(y_table)
    #print(data.y)
    ...
    #kmedoids = cluster.KMedoids(n_clusters=n_y_pred, method='pam')
    ...
    if (n_y_pred > 3):
        ...
        keyboard.wait("Enter")
    '''
    idx_attach = obtain_attach_nodes_by_cluster(args,y_table,y_pred,kmedoids,idx_val.cpu().tolist(),encoder_x,data.y,device,size).astype(int)
    print("obtain is over")
    return idx_attach
...
def cluster_degree_selection(args,data,idx_train,idx_val,idx_clean_test,unlabeled_idx,train_edge_index,size,device):
    ...
    if(os.path.exists(selected_nodes_path)):
        ...
        return idx_attach
    gcn_encoder = GCN_Encoder(args,'GCN_Encoder',data,device).to(device)
    t_total = time.time()
    # edge_weights = torch.ones([data.edge_index.shape[1]],device=device,dtype=torch.float)
    print("Length of training set: {}".format(len(idx_train)))
