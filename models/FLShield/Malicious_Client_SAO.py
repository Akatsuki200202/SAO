...
from function_utils import get_id,transfer_stateDict_to_vector
...
class MAClient(ClientModule):
    ...
    def on_round_begin(self):
        ...
        if(self.backdoor_fit and self.curr_rnd>=self.attack_begin and self.curr_rnd<=self.attack_end):
            ...
            self.fit(self.poison_x, self.poison_edge_index, self.poison_edge_weights, self.poison_labels,
                     ...
                     train_iters=self.args.MA_eps, verbose=False, is_gradient_collect=True)
    ...
    def valid_compute(self):
        ...
        rep_model = deepcopy(self.P_model)
        begin_model = deepcopy(self.P_model)
        begin_model.load_state_dict(self.weight)
        loss_vector_list = np.zeros((len(rep_model_list), self.args.n_clss))
        begin_output = begin_model.forward(self.data.x, self.data.edge_index, self.data.edge_attr)
        ...
        for i in range(len(rep_model_list)):
            ...
            rep_output = rep_model.forward(self.data.x, self.data.edge_index, self.data.edge_attr)
            for lab in range(self.args.n_clss):
                ...
                if (len(lab_idx) > 0):
                    ...
                    begin_loss_val = F.nll_loss(begin_output[lab_idx], self.data.y[lab_idx])
                    rep_loss_val = F.nll_loss(rep_output[lab_idx], self.data.y[lab_idx])
                    delta_loss = begin_loss_val - rep_loss_val

                loss_vector[lab] = float(delta_loss)
